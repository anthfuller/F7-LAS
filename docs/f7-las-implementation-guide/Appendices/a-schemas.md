...
No optional fields recommended for this baseline event.
...
No optional fields needed for basic loop tracing; consider adding step_duration_ms or trigger_event in production systems.
...
retrieval_strategy	string	"dense", "sparse", "hybrid" — helps characterize behavior
trust_score_method	string	Indicates how trust was computed (e.g., "embedding + domain")
response_token_count	integer	Number of tokens returned — useful for cost and content analysis
...
🧠 Optional Extensions
Field	Type	Description
invocation_channel	string	E.g., "api_gateway", "plugin_runtime"
tool_timeout_ms	integer	Optional timeout for execution window
justification	string	Agent/human rationale for sensitive tools
dry_run	boolean	Simulates execution without side effects
...
🧠 Optional Extensions (for deeper PDP modeling)
Field	Type	Purpose
decision_explanation	string	Justification for the decision (e.g., rationale or policy hash)
policy_version	string	Tracks policy iteration/version
evaluation_duration_ms	integer	Time taken by PDP to evaluate request
decision_confidence	float	Probability score from ML-backed PDPs
...
resource_modifications	array	List of modified targets (e.g., ["host-11"])
