...
This guidance applies **only** when MCP is intentionally part of an agent’s tool integration fabric and the organization chooses a structured enforcement point for MCP interactions.

Where MCP is not used — this guidance does not apply.
